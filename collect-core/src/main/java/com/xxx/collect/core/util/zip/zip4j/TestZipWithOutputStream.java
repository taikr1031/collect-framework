 ...
 */
public class TestZipWithOutputStream {

  public TestZipWithOutputStream() {
    ...
    try {
      ...
      filesToAdd.add(new File("c:\\ZipTest\\中文.txt"));

      ZipModel zipModel = new ZipModel();
      zipModel.setFileNameCharset("GBK");//不加此参数，360和winrar都可以识别中文，但是windows系统自带的无法识别中文
      ...
      FileOutputStream fileOutputStream = new FileOutputStream(new File("c:\\ZipTest\\CreateZipFileWithOutputStreams.zip"));
      outputStream = new ZipOutputStream(fileOutputStream, zipModel);
      ...
      for (int i = 0; i < filesToAdd.size(); i++) {
        ...
        outputStream.putNextEntry(new StreamEntry(file.getName(), new FileInputStream(file)), parameters);
